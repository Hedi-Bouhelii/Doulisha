...
| `booking.byReference` | query    | public | One own order: status, payments, tickets with QR codes once confirmed, refund if cancelled.                                                                                                                                                                    |
...
| `organizer.createProfile` | mutation | member  | Creates an organizer profile and grants the `organizer` role (unverified until an admin checks it).                                                                        |
...
| Procedure              | Type     | Access | Description                                                                                                                                                       |
| ---------------------- | -------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `uploads.create`       | mutation | public | Upload ticket for `event-cover`, `event-photo` (public, 5 MB) or `payment-proof` (private, 8 MB, PDF allowed). Type and size are checked before any byte is sent. |
